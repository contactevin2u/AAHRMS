...
async function showFullList() {
  // Get all outlets with IDs
  const outlets = await pool.query(`
    SELECT id, name FROM outlets WHERE company_id = 3 ORDER BY id
  `);

  console.log('='.repeat(80));
  console.log('OUTLET LIST WITH IDs');
  console.log('='.repeat(80));
  for (const o of outlets.rows) {
    console.log(`  ID ${String(o.id).padStart(2)} : ${o.name}`);
  }

  // Get all active employees with their current outlet
  ...
  // Expected assignments based on user's list (IC -> expected outlet/role/work_type)
  ...
  console.log('\n' + '='.repeat(100));
  console.log('EMPLOYEES NEEDING UPDATES');
  ...
  for (const emp of employees.rows) {
    ...
    if (needsOutletChange || needsRoleChange || needsWorkTypeChange) {
      ...
      let changes = [];
      if (needsOutletChange) changes.push(`outlet: ${emp.outlet_id || 'NULL'} -> ${expected.outlet_id}`);
      ...
      console.log(`\n${count}. ${emp.name}`);
      console.log(`   IC: ${ic}`);
      console.log(`   Current: ${emp.outlet_name || 'NO OUTLET'} (ID: ${emp.outlet_id || 'NULL'})`);
      console.log(`   Should be: ${expected.outlet} (ID: ${expected.outlet_id})`);
      ...
      updates.push({
        ...
        name: emp.name,
        ic: ic,
        new_outlet_id: expected.outlet_id,
        new_role: expected.role || null,
        new_work_type: expected.work_type || null
      });
    }
  }

  console.log('\n' + '='.repeat(100));
  console.log(`TOTAL: ${count} employees need updates`);
  console.log('='.repeat(100));

  process.exit(0);
}

showFullList().catch(e => { console.error(e); process.exit(1); });
